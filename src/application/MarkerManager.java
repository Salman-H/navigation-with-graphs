...
public class MarkerManager {
	...
	/**
	 * Used to set reference to visualization button. Manager will be
	 * responsible for disabling button
	 *
	 * @param vButton
	 */
	...
	public void restoreMarkers() {
		...
		while (it.hasNext()) {
			...
			// destination marker needs to be added because it is added in
			// javascript
	...
	public static MarkerOptions createDefaultOptions(LatLong coord) {
		...
		markerOptions.animation(null).icon(markerURL).position(coord).title(null).visible(true);
	...
	public void displayMarker(geography.GeographicPoint point) {
		if (markerMap.containsKey(point)) {
			...
			// System.out.println("no key found for
			// MarkerManager::displayMarker");
		}
	}
	...
	private void registerEvents(Marker marker, geography.GeographicPoint point) {
		 ...
		 * { marker.setVisible(true); //marker.setAnimation(Animation.BOUNCE);
		 * });
		 * 
		...
		map.addUIEventHandler(marker, UIEventType.click, (JSObject o) -> {
			...
			if (selectMode) {
				if (selectedMarker != null && selectedMarker != startMarker && selectedMarker != destinationMarker) {
